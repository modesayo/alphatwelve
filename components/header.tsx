...
interface NavItem {
  ...
  const Header = () => {
    ...
    const dropdownRef = useRef<HTMLDivElement>(null); // Reference to the dropdown for click detection
    ...
    return (
      ...
      <header className="flex fixed top-0 w-full z-10 justify-between items-center p-4 bg-gray-800 h-[64px] text-white">
        ...
        <div className="relative">
          ...
          >
            {isOpen ? 'X' : 'Menu'}  {/* Change button text based on dropdown state */}
          </button>
        </div>
      </header>
  
      {isOpen && (
        <div ref={dropdownRef} className="fixed top-[64px] z-10 left-0 h-full w-full bg-white text-black p-4">
          <ul>
            {navItems.map((item) => (
              <li key={item.id}>
                <a
                ...
                >
                  <div className="inline-flex items-center">
                    {item.icon1 && <span className="text-lg mr-2">{item.icon1}</span>}
                    {item.name}
                  </div>
                </a>
              </li>
